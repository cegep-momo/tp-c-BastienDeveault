...
// Display all books
void Library::displayAllBooks(string &typeTri)
{
    if (books.empty())
    {
    ...
    view.reserve(books.size());
    for (const auto &up : books)
    ...
    if (typeTri == "2")
    {
        sort(view.begin(), view.end(),
             ...
             {
                 return lower(a->getAuthor()) < lower(b->getAuthor());
             });
    }
    else
    {
        sort(view.begin(), view.end(),
             [&](const Book *a, const Book *b)
             {
                 return lower(a->getTitle()) < lower(b->getTitle());
             });
    }
    cout << "\n=== TOUS LES LIVRES ===\n";
